...
'MESAJ ÝÞLEMLERÝ
ElseIf act="SafeEditMsg" Then
act="safe":call editmsg
ElseIf act="EditMsg" Then
act="norm":call editmsg
...
Sub rep_update
IF Session("Level")="1" OR Session("Level")="5" OR Session("ISMod")="Yes" Then
IF Request.Form("msg")="" Then
Response.Write "<br /><center>Tüm Alanlarý Doldurunuz</center><br />"
ELSE
Set mRs=Connect.Execute("Select * FROM MESSAGES WHERE mid="&QS_CLEAR(Request.QueryString("id"),"false")&"")
frm_mid=mRs("mhit")
Connect.Execute("UPDATE MESSAGES SET mmsg='"&Request.Form("msg")&"' WHERE mid="&QS_CLEAR(Request.QueryString("id"),"false")&"")
...
call UST
IF Session("Level")="1" OR Session("Level")="5" OR Session("ISMod")="Yes" Then
Set rs=Server.CreateObject("ADODB.RecordSet"):rs.open "Select * FROM MESSAGES WHERE mid="&QS_CLEAR(Request.QueryString("id"),"false")&"",Connect,3,3%>
<form action="?action=Update-Rep&id=<%=QS_CLEAR(Request.QueryString("id"),"false")%>" method="post"><table border="0" cellspacing="2" cellpadding="2" width="50%" align="center" class="td_menu">
<tr bgcolor="<%=menu_color%>"><td><textarea name="msg" rows="10" class="form1" style="width:100%" cols="20"><%=rs("mmsg")%></textarea></td></tr>
<tr><td><input type="submit" value="Güncelle »" class="submit"></td></tr>
</table>
</form>
...
Set mRs=Connect.Execute("Select * FROM MESSAGES WHERE mid="&QS_CLEAR(Request.QueryString("id"),"false")&"")
frm_mid=mRs("mhit")
Connect.Execute("DELETE FROM MESSAGES WHERE mid="&QS_CLEAR(Request.QueryString("id"),"false")&"")
Set mRs2=Connect.Execute("Select * FROM MESSAGES WHERE mid="&frm_mid&"")
Set Rmsg=Connect.Execute("UPDATE MESSAGES SET cvp_sayisi='"&mRs2("cvp_sayisi")-1&"' WHERE mid="&frm_mid&"")
Response.Redirect "Forum.asp?action=Topic&id="&frm_mid&""
End IF
End Sub
...
ELSE
Set fRs=Server.CreateObject("ADODB.RecordSet")
fRs.open "Select * FROM CATEGORIES WHERE cid="&QS_CLEAR(Request.QueryString("forum"),"false")&"",Connect,3,3
Response.Write "<br /><div align=center class=td_menu><b>" & fRs("cname") & "</b> Forumu için Yönetici Silme Sayfasý</div>"
Session("SelectedForum")=QS_CLEAR(Request.QueryString("forum"),"false")
%>
...
Sub mod_new
IF Session("Level")="1" OR Session("Level")="5" Then
IF QS_CLEAR(Request.QueryString("step"),"false")="3" Then
...
fRs.Close
Set fRs=Nothing
ElseIF QS_CLEAR(Request.QueryString("step"),"false")="2" Then
...
umSQL="Select * FROM MESSAGES WHERE mid="&QS_CLEAR(Request.QueryString("id"),"false")&""
...
IF Session("Level")="1" OR Session("Level")="5" OR Session("ISMod")="Yes" Then
call UST
id=QS_CLEAR(Request.QueryString("id"),"false"):Set msg=Connect.Execute("Select * FROM MESSAGES WHERE mid="&id&"")%>
<br /><br />
<form action="?action=UpdateMsg&id=<%=id%>" method="post">
...
<td width="30%" align="right">Baþlýk</td>
<td width="70%"><input type="text" name="m_title" size="60" class="form1" value="<%=msg("mtitle")%>"></td>
</tr>
<tr bgcolor="<%=top_menu_color%>">
<td valign="top" align="right">Mesaj</td>
<td>
<%if act="norm" then%><script src="htmlarea/editor.js" language="Javascript1.2"></script><script language="JavaScript1.2" defer>editor_generate('m_msg');</script><%end if%>
<textarea name="m_msg" rows="10" cols="60" class="form1"><%=msg("mmsg")%></textarea>
</td>
</tr>
<tr bgcolor="<%=top_menu_color_hover%>">
<td colspan="2" align="center"><input type="submit" value="Deðiþiklikleri Kaydet >>>" class="submit">&nbsp;&nbsp;&nbsp;&nbsp;
<input type="button" class="submit" value="Ý P T A L" onClick="window.location.href='Forum.asp?action=Topics&id=<%=msg("cid")%>'"</td>
</tr>
</table>
</form>
<%call ALT
...
Sub mainupdate
IF Session("Level")="1" OR Session("Level")="5"  Then
Set updCat=Connect.Execute("UPDATE MAIN_CATS SET mc_name='"&duz(Request.Form("c_name"))&"' WHERE mcid="&QS_CLEAR(Request.QueryString("id"),"false")&"")
...
IF Session("Level")="1" OR Session("Level")="5" Then
call UST
Set rs=Connect.Execute("Select * FROM MAIN_CATS WHERE mcid="&QS_CLEAR(Request.QueryString("id"),"false")&"")
%>
<br /><br />
<form action="?action=UpdateMain&id=<%=QS_CLEAR(Request.QueryString("id"),"false")%>" method="post">
...
Sub maindel
IF Session("Level")="1" OR Session("Level")="5" Then
mcid=Request.QueryString("id"):mcid=QS_CLEAR(mcid,"false")
Set d1=Server.CreateObject("ADODB.RecordSet"):d1SQL="DELETE * FROM MAIN_CATS WHERE mcid="&mcid&"":d1.open d1SQL,Connect,3,3
Set dsc=Connect.Execute("Select * FROM CATEGORIES WHERE mcid="&mcid&"")
If NOT dsc.Eof Then
Set d2=Server.CreateObject("ADODB.RecordSet"):d2SQL="DELETE * FROM MESSAGES WHERE cid="&dsc("cid")&"":d2.open d2SQL,Connect,3,3
Set d3=Server.CreateObject("ADODB.RecordSet"):d3sQL="DELETE * FROM CATEGORIES WHERE mcid="&mcid&"":d3.open d3SQL,Connect,3,3
End If
...
Sub delmsg
IF Session("Level")="1" OR Session("Level")="5" OR Session("ISMod")="Yes" Then
msid=Request.QueryString("id"):msid=QS_CLEAR(msid,"false")
Set mdel=Server.CreateObject("ADODB.RecordSet"):mdSQL="DELETE * FROM MESSAGES WHERE mid="&msid&" AND topic=True":mdel.open mdSQL,Connect,3,3
Set mdel2=Server.CreateObject("ADODB.RecordSet"):md2SQL="DELETE * FROM MESSAGES WHERE mhit="&msid&" AND topic=False":mdel2.open md2SQL,Connect,3,3
Response.Redirect Request.ServerVariables("HTTP_REFERER")
End IF
End Sub
...
Sub noentry
IF Session("Level")="1" OR Session("Level")="5" Then
cid=Request.QueryString("id"):cid=QS_CLEAR(cid,"false")
...
Set del1=Server.CreateObject("ADODB.RecordSet"):d1SQL="DELETE FROM CATEGORIES WHERE cid="&id&"":del1.open d1SQL,Connect,3,3
Set del2=Server.CreateObject("ADODB.RecordSet"):d2SQL="DELETE FROM MESSAGES WHERE cid="&id&"":del2.open d2SQL,Connect,3,3
Response.Redirect Request.ServerVariables("HTTP_REFERER")
End IF
End Sub
...
IF Session("Level")="1" OR Session("Level")="5" Then
call UST
Set rs=Connect.Execute("Select * FROM CATEGORIES WHERE cid="&QS_CLEAR(Request.QueryString("id"),"false")&"")
Set cats=Connect.Execute("Select * FROM MAIN_CATS")%>
<form action="?action=UpdateCat&id=<%=QS_CLEAR(Request.QueryString("id"),"false")%>" method="post"><table border="0" cellspacing="2" cellpadding="2" align="center" width="70%" style="font-family:verdana; font-size:11px;">
<tr><td width="30%" align="right">Forum Adý : </td><td width="70%" align="left"><input type="text" name="cname" class="form1" size="30" value="<%=rs("cname")%>"></td></tr>
<tr><td width="30%" align="right">Forum Açýklamasý : </td><td width="70%" align="left"><textarea name="cinfo" rows="5" cols="29" class="form1"><%=rs("cinfo")%></textarea></td></tr>
<tr><td width="30%" align="right">Forum Kategori : </td><td width="70%" align="left"><select name="mcat" size="1" class="form1"><%Do While NOT cats.Eof
If rs("mcid")=cats("mcid") Then sel="selected" Else sel=""%><option value="<%=cats("mcid")%>" <%=sel%>><%=cats("mc_name")%></option><%cats.MoveNext
Loop%></select></td></tr>
<tr><td width="30%" align="right">&nbsp;</td><td width="70%" align="left"><input type="submit" class="submit" value="G ü n c e l l e"></td></tr>
</table></form>
<%rs.Close:Set rs=Nothing
call ALT
End IF
...
Set cupd=Server.CreateObject("ADODB.RecordSet"):cupdSQL="Select * FROM CATEGORIES WHERE cid="&id&"":cupd.open cupdSQL,Connect,3,3
...
Sub unlock
IF Session("Level")="1" OR Session("Level")="5" Then
fid=Request.QueryString("id"):fid=QS_CLEAR(fid,"false")
Set upd=Server.CreateObject("ADODB.RecordSet"):updSQL="SELECT * FROM CATEGORIES WHERE cid="&fid&"":upd.open updSQL,Connect,3,3
If NOT upd.Eof Then
upd("locked")=False
upd.Update
End IF
upd.Close:Set upd=Nothing
Response.Redirect Request.ServerVariables("HTTP_REFERER")
End IF
End Sub
